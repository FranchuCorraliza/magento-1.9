...
function inicio(){
	...
	try{
		...
		foreach($files as $file){
				...
				$importacion_outlet	= importar_extra($dir_processing,$file,'zona2_en');
				...
				if($contaLineas==0):
					...
					foreach($data as $valor){
						...
						if($item['short_description']==''):
							$item['short_description'] 	= getDescripcionCorta($data[$posicion_campo_tres],$data[$posicion_nombre]);
						endif;
		...
		if($handle){
			...
			while (($data = fgetcsv($handle, 6000, ';')) !== FALSE) {
				if($contaLineas==0):
					...
					try{
						echo $contaLineas.".-procesando-->".$item['sku']." || ".$data[$posicion_campo_uno];
						echo "<hr>";
						$dp->ingest($item); //insert item
						unset($item); 		//clean item
					} catch (Exception $ex){ 
