...
class Amasty_Geoip_Adminhtml_ImportController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function startAction()
    {
        ...
        try {
            ...
            $ret = Mage::getModel('amgeoip/import')->startProcess($type, $filePath, $this->_geoipIgnoredLines[$type]);
    ...
    public function commitAction()
    {
        ...
        try {
            $type = $this->getRequest()->getParam('type');
            $import = Mage::getModel('amgeoip/import');
            $import->commitProcess($type);
            $result['status'] = 'done';
            $result['full_import_done'] = Mage::getModel('amgeoip/import')->isDone() ? "1" : "0";
        } catch (Exception $e) {
            $result['error'] = $e->getMessage();
        }
