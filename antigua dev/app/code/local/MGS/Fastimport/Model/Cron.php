<?php 
set_time_limit(0);
class MGS_Fastimport_Model_Cron{
		...
		if($handle){
			...
			while (($data = fgetcsv($handle, 6000, ';')) !== FALSE) {
				...
				if($contaLineas==0):
					...
					$product_id = $read->fetchOne("select entity_id from catalog_product_entity where sku='".$sku."'");
 	 		
				 	if($product_id!='' and $data[$posicion_stock]!=''):
				 	 	$this->_updateProductQty ($product_id,$data[$posicion_stock]);
					endif;
