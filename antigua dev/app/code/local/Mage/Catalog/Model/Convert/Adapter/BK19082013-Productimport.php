...
extends Mage_Catalog_Model_Convert_Adapter_Product
 {
    ...
    */
	...
	//***************************************************************************************************
	// Código añadido para impedir que se inserten nuevas imágenes en las importaciones siguientes a la primera
         	
			$finrutaimagen = substr($importData['rutaimagen'],strlen(strtolower($importData['rutaimagen']))-5);
$finrutaimagen2 = substr($importData['campo_cuatro'],strlen(strtolower($importData['campo_cuatro']))-5);
$finrutaimagen3 = substr($importData['campo_cinco'],strlen(strtolower($importData['campo_cinco']))-5);
			...
			|| (($product -> getMediaGalleryImages() -> getSize()==1) && $finrutaimagen2=='1.jpg') /* Si se va ha subir la 2ª imagen de un producto con 2 imagenes */
        	...
        	$ruta = Mage :: getBaseDir( 'media' ) ."/". 'import' . str_replace("/media/import","",$importData['rutaimagen']); //modificado para pillar la imagen de ELITE
			//print_r($ruta); 
	...
	//***********************************************************************************************************
	//Añadimos la categoría rebajas o novedades en función del tipo del producto	
	
		if (isset($importData['tipo'])){
			
			if ($importData['tipo']=='132'){
				$categorias[]='206';
			}else if ($importData['tipo']=='131' && $importData['special_price']!='.00'){
				$categorias[]='173';
			}
		}
	//************************************************************************************************************
		...
		//print_r($importData);
		 foreach($categorias as $categor):
			$cats .= $categor.",";
		 endforeach;
		 echo $cats = substr($cats,0,-1);
		// echo $ruta;
		 //$product->setManufacturer($importData["manufacturer"]);
		 
		 echo "---->".$manufacturer = $this->getAttributeValue('Manufacturer',$importData["manufacturer"]);
		 echo $ruta;
