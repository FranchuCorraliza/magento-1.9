...
class Mage_Servired_StandardController extends Mage_Core_Controller_Front_Action
{
    ...
    public function redirectAction()
    {
        ...
        if($session->getLastOrderId())
        {
            ...
            if(!$ya_en_TPV)
            {
                 ...
                 * Items de la orden
                 */
                /* Comentamos estas lineas para utilizar la actualización de stocks de magento */
				/*
    ...
    public function cancelAction()
    {
        ...
        if(count($params) > 0)
        {
			...
			echo ("entra en count(param)<br/>");			
            if($this->firmaValida($params))
            {
				echo ("entra en firmaValida<br/>");				
                ...
                $this->_redirect('checkout/cart');
					...
					echo ("entra en no firmaValida<br/>");
            {
                /**
				...
				echo ("entra en no count(param)<br/>");
        {
            /**
            ...
            if($this->firmaValida($params))
            {
                ...
                if($autorizada == true)
                {
                    ...
                    if($order->getId())
                    {
						...
						/* Comentamos esto para utilizar la reducción de stocks de Magento */
                ...
                } else
                {
					...
					/**
					* Si la transaccion fue denegada devolvemos el stock de los artículos
					*/
					$items = $order->getAllItems();
                        if($items)
                        {
