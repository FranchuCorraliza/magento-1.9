    ...
    public function beforeSendShipment ($observer)
    {
		...
		if (count($_history)):
			...
			foreach ($_history as $_historyItem):
			 if ($_historyItem->getFileId()):
			 	if ($_historyItem->getFileId()>$fileId):
					$fileId=$_historyItem->getFileId();
				endif;
			 endif;
			endforeach;
			$files = Mage::getSingleton('orderspro/upload_files')->load($fileId);
			$helper = Mage::helper('orderspro');
			if ($file = $helper->isUploadFile($fileId)){
...
//			$pdf = Zend_pdf::load("http://dev.elitestore.es/index.php/orderspro/dl/file/id/10/T72F-141529.pdf");
//			$pdf->save(); 
			//$pdf= Zend_Pdf::load($this->helper('orderspro')->getUploadFilesPath(10, false));
			//$pdf= Zend_Pdf::load('orderspro/dl/file/id/10/T72F-141529.pdf');
			
			//$helper->getFile($file, $files->getFileName());
			//$pdf = Mage::getModel('emailattachments/order_pdf_order')->getFacturaPdf(array($order));
