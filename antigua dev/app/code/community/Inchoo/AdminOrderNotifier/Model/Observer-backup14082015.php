...
class Inchoo_AdminOrderNotifier_Model_Observer extends Mage_Core_Helper_Abstract
{
    ...
    {  
		...
		foreach ($order->getStatusHistoryCollection(true) as $_item): //Si se ha enviado la notificación a la tienda anteriormente (pedidos Uncleared, bancktransfer o Paypal Payment Review) no se vuelve a enviar 
			if ($_item->getStatusLabel()=='26.- Reservar en Tienda' || $_item->getStatusLabel()=='Payment Review'):
				$notificar_tiendas=false;				
			endif;
		endforeach;
        if (!$helper->isModuleEnabled($storeId) || ($invoice_count>1) || !$notificar_tiendas) {
            return;
        }
