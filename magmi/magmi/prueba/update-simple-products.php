...
function inicio(){
	try{
		...
		foreach($files as $file){
				Mage::log("trabajamos con ".$dir_processing.$file, null, 'direct-sql.log');
				echo "procesamos ".$dir_processing.$file;
		...
		//REINDEX DEACTIVATED BY DEFAULT
			try{
				...
				Mage::log("ERROR REINDEX -> ".$ex->getMessage(), null, 'direct-sql.log');
		...
		Mage::log("Final de importacion, duración: ".$minutos." minutos  / ".$segundos." segundos", null, 'direct-sql.log');


	} catch (Exception $ex){ 
			Mage::log("ERROR EN LECTURA DE FICHEROS ".$ex->getMessage(), null, 'direct-sql.log');
	...
	if ($visibleweb=='T'){
		$websites = "e_stock_es";
			...
			if($product){
				...
				if (count($tiendas)>1){ //En el caso en que se hubiese puesto en las dos tiendas desde magento.
					$websites = "es,e_stock_es";
				}else{
					$websites= "es";
				}
			} else {
				$websites= "admin"; //añadido nuevo no tengo producto es nuevo y no estaba asignado @todo: preguntar como funcionaba esto
			}						
	}else{
...
function reindex(){
	...
	for($i=0;$i<sizeof($reindexar);$i++):
		try{
			...
			Mage::log("No Puedo reindexar: ".$ex->getMessage(), null, 'direct-sql.log');
		}
	endfor;
	...
	Mage::log("Reindexado duración: ".$minutosReindex." minutos  / ".$segundosReindex." segundos", null, 'direct-sql.log');
	...
	exit($error);
		try{
			...
			->send();	
				...
				Mage::log("No Puedo enviar email: ".$ex->getMessage(), null, 'direct-sql.log');
		}
}
//QUITAR QUOTES
