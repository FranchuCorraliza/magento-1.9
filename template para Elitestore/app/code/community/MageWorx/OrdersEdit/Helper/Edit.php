...
class MageWorx_OrdersEdit_Helper_Edit extends Mage_Core_Helper_Abstract
{
    ...
    {
        if (version_compare(Mage::getConfig()->getModuleConfig("Mage_Sales")->version, '1.6.0.8', '>=') || (version_compare(Mage::getConfig()->getModuleConfig("Mage_Sales")->version, '1.6.0.8', '<') && $orderItem->getProductType() != Mage_Catalog_Model_Product_Type::TYPE_BUNDLE)){
            if (!($orderItem->getQtyToCancel() || $orderItem->getQtyToRefund())) {
                return false;
            }
        }
        return true;
