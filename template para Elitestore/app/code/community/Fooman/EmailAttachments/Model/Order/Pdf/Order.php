...
//            $page = $this->insertTotals($page, $order);
        ...
        foreach ($order->getAllItems() as $item) {
			...
			if (!is_null($image)) {
				$imagePath = Mage::helper('catalog/image')->init($image, 'image')
						...
						->resize(300, 450)
						->__toString();
				$imageLocation = substr($imagePath,strlen(Mage::getBaseUrl())-7);
				/*$this->insertImage($imageLocation, 35, (int)($this->y + 15), 125, (int)($this->y+105), $width, $height, $page);*/
				$this->insertImage($imageLocation, 35, (int)($this->y + 15), 95, (int)($this->y+105), $width, $height, $page);
			}
			$codbarras=Mage::getModel('catalog/product')->loadByAttribute('sku', $item->getSku())->getData('codbarras');
			
			if ($codbarras!=''){
				//$log=exec(Mage::getBaseUrl().'media/barcode/sample-gd.php?code='.Mage::getModel('catalog/product')->loadByAttribute('sku', $item->getSku())->getData('codbarras'));
				$barcode_image='media/barcode/sample-gd.php?code='.Mage::getModel('catalog/product')->loadByAttribute('sku', $item->getSku())->getData('codbarras');
				$this->insertImage($barcode_image,500,$this->y+40,800,$this->y+90,300,50,$page);
			}
    ...
    protected function _printTableHead($page)
    {
		...
		$page->drawText(Mage::helper('sales')->__('Products'), 130, $this->y, 'UTF-8');
        $page->drawText(Mage::helper('sales')->__('SKU'), 300, $this->y, 'UTF-8');
		$page->drawText(Mage::helper('sales')->__('Qty'), 450, $this->y, 'UTF-8');
        $page->drawText(Mage::helper('sales')->__('Barcode'), 500, $this->y, 'UTF-8');
