...
class Mirasvit_Fpc_Helper_CustomDependence extends Mage_Core_Helper_Abstract
{
     ...
     {
		if (Mage::helper('mstcore')->isModuleInstalled('Sandfox_GeoIP')) {
             $geoipCountry = Mage::getSingleton('geoip/country');
			 $cookie = Mage::getSingleton('core/cookie');
			 $banderita = $cookie->get('geoip_bandera');
             $country = $geoipCountry->getCountryByIp(Mage::helper('core/http')->getRemoteAddr());
			 $mobile="desktop";
			 if (Mage::helper('mobiledetect')->isMobile()){
				 $mobile="mobile";
			 }
             return $country."_".$banderita."_".$mobile;
//             return $geoipCountry->isCountryAllowed($country) . $geoipCountry->isDefaultCountry($country);
        }
		return true;
     }
