...
if (!$installer->getConnection()->tableColumnExists($installer->getTable('sales/order'), 'order_group_id')) {
    ...
    if ($installer->getConnection()->showTableStatus($installer->getTable('orderspro_order_item_group'))) {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $select = $connection->select()->from($installer->getTable('orderspro_order_item_group'), array('order_id', 'order_group_id'));
        ...
        if ($allOrderGroups) {
            ...
            $installer->run('UNLOCK TABLES;
                DROP TABLE IF EXISTS `'. $installer->getTable('orderspro_order_item_group') .'`;
                ALTER TABLE `'. $installer->getTable('orderspro_order_group') .'` CHANGE `order_group_id` `order_group_id` TINYINT UNSIGNED NOT NULL AUTO_INCREMENT;');
        }
    }
}
