...
class MageWorx_Adminhtml_Block_OrdersPro_Sales_Order_Grid_Renderer_Comments extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{

    public function render(Varien_Object $row) {
        $comments = explode("\n", $this->htmlEscape($row->getData($this->getColumn()->getIndex())));
        $comments = array_reverse($comments);
        $limit = Mage::helper('orderspro')->getNumberComments();
        ...
        if ($count>3) {
            $comments[$count-1].='<a href="" onclick="$(\'hdiv_'.$row->getData('increment_id').'_'.$prefix.'\').style.display=\'none\'; $(\'a_'.$row->getData('increment_id').'_'.$prefix.'\').style.display=\'block\'; return false;" style="float:right; font-weight:bold; text-decoration: none;" title="'. Mage::helper('orderspro')->__('Less..') .'">↑</a>'
                .'</div>'
