...
class Elite_Urlrewrites_Model_Observer {
	...
	private function _createBrandsCategoryPages($product){
	...
	private function _getCategoriasAptas($product){
	...
	private function _createUrlPath($categoryId, $manufacturerId){
		if($manufacturerId!=""){
			$category=Mage::getModel("catalog/category")->load($categoryId);
			
			if ($category->getParentCategory()->getLevel()>1){
				$this->_createUrlPath($category->getParentCategory()->getId(),$manufacturerId);
			}
