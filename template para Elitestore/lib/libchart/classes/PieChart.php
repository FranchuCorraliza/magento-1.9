        ...
        class PieChart extends Chart
        {
                ...
                */
                ...
                function createImage()
                {
                        ...
                        // Legend box
						$addL = 4;
						$correctTLY = $this->pieTLY - $this->correctTLY;
						$correctBRY = $this->pieBRY + $this->correctBRY;
						
						$addR = 4;
						$correctTLX = $this->pieTLX - $this->correctTLX;
						$correctBRX = $this->pieBRX + $this->correctBRX;
						

                        $this->outlinedBox($correctTLX, $correctTLY, $correctBRX, $correctBRY);

                        // Aqua-like background

                        $aquaColor = Array($this->aquaColor1, $this->aquaColor2, $this->aquaColor3, $this->aquaColor4);

                        for($i = $correctTLY + 2; $i < $correctBRY; $i++)
                        {
                            $color = $aquaColor[($i + 3) % 4];
                            $this->primitive->line($correctTLX + 2, $i, $correctBRX - 2, $i, $color);
                        }
                }
				
				function setOutlinedBoxMarginLeft($correctTLY)
				{
					$this->correctTLY = $correctTLY;					
				}
				
				function setOutlinedBoxMarginRight($correctBRY)
				{
					$this->correctBRY = $correctBRY;					
				}
				
				function setOutlinedBoxMarginTop($correctTLX)
				{
					$this->correctTLX = $correctTLX;
				}
				
				function setOutlinedBoxMarginBottom($correctBRX)
				{
					$this->correctBRX = $correctBRX;
				}
