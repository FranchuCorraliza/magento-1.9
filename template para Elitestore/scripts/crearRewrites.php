...
if (!$_GET['sku']){
	?>
		<div>
		<form>
			<label for="sku" >Inserta el sku</label>
...
function createUrlPathEn($categoryId, $manufacturerId,$manufacturerName){
		if($manufacturerId!=""){
			...
			echo "-->Id:$id_path";
			echo " || ".Mage::getModel('core/url_rewrite')->loadByIdPath($id_path)->getId();
			...
			echo " || ".Mage::getModel('core/url_rewrite')->loadByRequestPath($requestPath)->getId();
			if (!(Mage::getModel('core/url_rewrite')->loadByIdPath($id_path)->getId()) && !(Mage::getModel('core/url_rewrite')->loadByRequestPath($requestPath)->getId())):
					try{
						...
						echo "Error al crear $requestPath";
						var_dump($e);
						echo "<hr>";
					}
