...
try{
	...
	if(file_exists ($file)){
		...
		$dp->endImportSession();
		echo "<hr>Actualización de stocks finalizada satisfactoriamente<hr>";
		$log=fopen($dir_processing."result-stocks-conf.log", "w");
		$txt = "1";
		fwrite($log, $txt);
		fclose($log);
	}else{
		$log=fopen($dir_processing."result-stocks-conf.log", "w");
		echo "<hr>No se ha encontrado el fichero para importar $dir_processing/$fileName<hr>";
		$txt = "0";
		fwrite($log, $txt);
		fclose($log);
	}
}catch(Exception $e){
	echo "<hr>Se produjo un error durante el proceso de actualización de stocks: $e<hr>";
	$log=fopen($dir_processing."result-stocks-conf.log", "w");
	$txt = "0";
	fwrite($log, $txt);
	fclose($log);
