...
function inicio(){
	...
	try{
		...
		foreach($files as $file){
				...
				//import FOTOS
				Mage::log("Comienzo importacion fotos", null, 'direct-sql-'.date("Ymd").'.log');
				$importacion_fotos	= importar_fotos($dir_processing,$file);
				
				//import to english version EN
				Mage::log("Comienzo importacion Ingles", null, 'direct-sql-'.date("Ymd").'.log');
				$importacion_english= importar_extra($dir_processing,$file,'en');
				
				//import to outlet version ES
				Mage::log("Comienzo importacion Outlet", null, 'direct-sql-'.date("Ymd").'.log');
				$importacion_outlet	= importar_extra($dir_processing,$file,'e_stock_es');
				
				//import to outlet version EN
				Mage::log("Comienzo importacion Outlet Ingles", null, 'direct-sql-'.date("Ymd").'.log');
		...
		if($handle){
			...
			while (($data = fgetcsv($handle, 6000, ';')) !== FALSE) {
				...
				if($contaLineas==0):
					...
					//eliminamos la columna store que nos mandan ya que tenemos que leer de visibleweb
					$borrar_key_2 = array_search('store', $cabecerasLinea);
					unset($cabecerasLinea[$borrar_key_2]);
					
					//eliminamos la columna is_in_stock que nos mandan ya que tenemos que leer de visibleweb
					$borrar_key_3 = array_search('is_in_stock', $cabecerasLinea);
					unset($cabecerasLinea[$borrar_key_3]);
					
					//eliminamos la columna associated
					$borrar_key_4 = array_search('associated', $cabecerasLinea); 
					unset($cabecerasLinea[$borrar_key_4]);
					...
					foreach($data as $valor){
						...
						if (array_key_exists($contaCampos,$cabecerasLinea)){
						...
						$item['category_ids']		= getCategorias($sku,$dto,$seccion,$familia);
						...
						//$item['stock'] 			= ""; // de que columna sacamos el stock???'
						//configurar imagenes con la ruta del servidor esto se indica en el conf de magmi
						//funcion mass update y rewrite omitidas lo hace magmi en el conf

						$item['status'] 			= getEstado($data[$posicion_status]);
						$item['visibility'] 		= getVisibilidad($data[$posicion_visibility]);
						
						if($item['short_description']==''):
							$item['short_description'] 	= getDescripcionCorta($data[$posicion_campo_tres],$data[$posicion_name]);
						endif;
						...
						if (!$product): //Asignamos peso únicamente si es la primera vez que se crea el producto
							$item['weight']				= getPeso($dto,$seccion,$familia);
						else:	//Si ya estaba creado asignamos el valor que tuviese antes
							$item['weight']				= $product->getWeight();
						endif;
						
						
						if(!$product):
							if($data[$posicion_visibleweb]=='F'):
								$item['store'] = "es"; 			
								$acumulador_sites[$sku] = "es";
							elseif($data[$posicion_visibleweb]=='T'): 
								$acumulador_sites[$sku] = "e_stock_es";
								$item['store'] = "e_stock_es";  
							else: //esto son los configurables que no traen
								$ids_csv 	= $data[$posicion_associated];
								$items 		= explode(",",$ids_csv);
								foreach($items as $sku){
									if($acumulador_sites[$sku]!=''):
										$item['store'] = $acumulador_sites[$sku];  //lo pasamos sin stores sino asocia todos	
									endif;
								}
							endif;
						else:
							$item['store'] = "admin"; //si ya existe actualizamos sobre admin
						endif;
						
						$item['url_key'] 			= getUrl($data[$posicion_name],$data[$posicion_tipo],$item['visibility'],$sku);
						
						$item['temporada'] 			= $data[$posicion_temporada];
						...
						//performace from reindex and mysql
						//1º delete categories from not visible individual item
						if($item['visibility']=='1'):
							$item['category_ids']='';
						endif;
						
						$item['configurable_attributes'] 	= reorderConfigAttributes($data[$posicion_config_attributes]);
						
						echo "<br>Type ->".$data[$posicion_type];
						
						if($data[$posicion_type]=='configurable'):						
							if(!in_array($data[$posicion_sku],$acumulador_asociados)){
								//echo "<br>-> no lo tengo en el array ".$data[$posicion_sku];
								$acumulador_asociados[]= $data[$posicion_sku];
								//echo "<br>->inserto en el array asociados a ".$data[$posicion_sku];
								$cadena_skus 			= getAssociated($data[$posicion_sku], $data[$posicion_associated]);
								$item['associated'] 	= $cadena_skus;
								$valor_asociado[$sku] 	= $cadena_skus;
								$item['simples_skus'] 	= $cadena_skus;
							} else {
						 		//echo "<br>Lo tengo en el array ya ";
						 		//print_r($acumulador_asociados);
								if (array_key_exists($data[$posicion_sku],$valor_asociado)){
									$item['associated'] 	= $valor_asociado[$data[$posicion_sku]];
									$item['simples_skus']  	= $valor_asociado[$data[$posicion_sku]];
								}
							}
						endif;
			...
			while (($data = fgetcsv($handle, 6000, ';')) !== FALSE) {
				if($contaLineas==0):
					...
					if($store_id=='e_stock_es'){
						...
						$item['news_from_date']		= getFechaNovedad($data[$posicion_sku]); //@jesus esto viene en el fichero porque se recalcula????
						$item['news_to_date']		= getFechaFinNovedad($data[$posicion_sku]); //@jesus esto viene en el fichero porque se recalcula????
					}
								...
								if(count($skus)>0):
									foreach($skus as $sku){
										...
										try{
											...
											if($_product):
												if (Mage::getModel('cataloginventory/stock_item')->loadByProduct($_product)->getIsInStock()):
													$almacen 	= 1;
													break;
												endif;
											endif; 
...
function getCategorias($sku,$departamento,$seccion,$familia){
			...
			$catalogo[4][1][1]=array(102);//no existe en el listado
			...
			$catalogo[4][3][1]=array(228,260);
			$catalogo[4][3][2]=array(229,261);
			...
			$catalogo[6][1][1]=array(124);
			$catalogo[6][1][2]=array(125);
			$catalogo[6][1][3]=array(126);
			$catalogo[6][1][4]=array(127);
			$catalogo[6][1][5]=array(128);
			$catalogo[6][1][6]=array(129);
			$catalogo[6][1][7]=array(130);
			$catalogo[6][1][8]=array(132);
			$catalogo[6][1][9]=array(133);
			$catalogo[6][1][10]=array(134);
			$catalogo[6][1][11]=array(135);
			$catalogo[6][1][12]=array(136);
			$catalogo[6][1][13]=array(137);
			$catalogo[6][1][14]=array(138);
			$catalogo[6][1][15]=array(140);
			$catalogo[6][2][1]=array(144);
			$catalogo[6][2][2]=array(145);
			$catalogo[6][3][1]=array(149);
			$catalogo[6][4][1]=array(151);
...
function getFechaNovedad($sku){
	...
	if(!$product){  //Si no existia antes
		...
		$fecha_final = $date->toString('Y-m-d');
	}else{
...
function getPeso($departamento,$seccion,$familia){
	...
	$catalogo[7][4][1]='2';	
	$weight='5';
	if ($departamento && $seccion && $familia && ($catalogo[$departamento][$seccion][$familia]!=null)){	
...
function reindexPrice(){
	...
	try
		exec('php ../shell/indexer.php -reindex catalog_product_price');
	} catch (Exception $ex){
...
/*function reindex(){
	...
	if(date("i")>0 and date("i")<16):
		...
		$reindexar[] = 'catalogsearch_fulltext';
	else:
		$reindexar[] = 'catalog_product_price';
		$reindexar[] = 'cataloginventory_stock';
		$reindexar[] = 'catalog_category_product';
